...
with open('data.txt') as sect_file:
    for line in sect_file:
        ...
        if int(partner1[i][0]) >= int(partner2[i][0]) and int(partner1[i][0]) <= int(partner2[i][1]):
            within += 1
            print("Partner pair: "+str(i)+" Partner 1: "+section[i][0]+" - Partner 2: "+section[i][1],end="\n") 
        elif int(partner1[i][1]) >= int(partner2[i][0]) and int(partner1[i][1]) <= int(partner2[i][1]):
            within += 1
            print("Partner pair: "+str(i)+" Partner 1: "+section[i][0]+" - Partner 2: "+section[i][1],end="\n") 
        elif int(partner2[i][0]) >= int(partner1[i][0]) and int(partner2[i][0]) <= int(partner1[i][1]):
            within += 1
            print("Partner pair: "+str(i)+" Partner 1: "+section[i][0]+" - Partner 2: "+section[i][1],end="\n")
        elif int(partner2[i][1]) >= int(partner1[i][0]) and int(partner2[i][1]) <= int(partner1[i][1]):
            within += 1
            print("Partner pair: "+str(i)+" Partner 1: "+section[i][0]+" - Partner 2: "+section[i][1],end="\n")
